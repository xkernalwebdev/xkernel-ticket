...
def send_ticket_email(email, name, tid, qr_path, event):
    try:
        print("EMAIL START")
        ...
        msg.attach(MIMEText(f"Hello {name}, your ticket {tid} is attached.", "plain"))

        with open(qr_path, "rb") as f:
            img = MIMEImage(f.read())
            msg.attach(img)

        server = smtplib.SMTP(app.config["MAIL_SERVER"], int(app.config["MAIL_PORT"]))
        server.starttls()
...
def home():
    ...
    if form.validate_on_submit():
        ...
        send_ticket_email(form.email.data, form.name.data, tid, path, form.event.data)

        flash("Ticket generated!")
    ...
    if "file" in request.files:
        ...
        if allowed_file(file.filename):
            ...
            for _, r in df.iterrows():
                ...
                send_ticket_email(r["Email"], r["Name"], tid, qr_path, r["Event Name"])

            os.remove(path)
            flash("Excel processed!")
...
def verify():
    ...
    tickets.update_one({"ticket_id": tid}, {"$set": {"used": True, "scanned_at": datetime.utcnow()}})
